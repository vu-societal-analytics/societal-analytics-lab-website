---
title: "Vacancy: Student Assistant – The AI Dilemma: an online game"
date: 2026-06-01
weight: 7
---

Apply before August 7th, 2026.
<!--more-->

# Overview and tasks

As student assistant, you will translate a physical serious game into an online one. The serious game is an adaptation of “The AI Dilemma” [https://zenodo.org/records/20160185](https://zenodo.org/records/20160185). 
The AI Dilemma is a collaborative game designed to be played simultaneously by two to five players plus a moderator. At its core, the game should allow players to:

* Randomly select cards.
* Move their tokens across the board.
* Throw a die.
* See each others progress.

In terms of software development:

* The game should be fully documented in GitHub.
* The game should be in a Docker container, so that other institutions can easily install it.
* The game will be securely stored in a VU server. 
* Players can easily access the game via a secure browser connection and with given credentials.
* The game should be easy to adapt to other settings, e.g., it should be easy to change the text in the game cards.


# What we offer

* You will be part of the Societal Analytics Lab [https://societal-analytics.nl/](https://societal-analytics.nl/).
* 3-months contract of 0.2 FTE starting in September 15th, 2026.
* Salary scale of 103.
* A space at the Communication Department to work.
* Access to a VU server to perform experiments.

# Your profile

## In terms of you:
* You are a master student registered at a Dutch higher education institution and you are allowed to work in the Netherlands.
* You have at least 60 ECTS.
* You don't exceed the max FTE you are allowed to work as student assistants on an annual basis, which is 0,4 FTE on average.

## In terms of your skills:
* You have experience in JavaScript.
* You have followed courses on or have experience with software design and development.
* You are a highly motivated student looking for opportunities to continue developing your computational skills.
* You are not afraid of tackling new challenges.
* You are not afraid of asking for help when things are not going as expected.
* You are good at managing your time and deadlines.

As a university, we strive for equal opportunities for all, recognizing that diversity takes many forms. We believe that diversity in all its complexity is invaluable for the quality of our teaching, research and service. We are always looking for talent with diverse backgrounds and experiences. This also means that we are committed to creating an inclusive community so that we can use diversity as an asset.

We realize that each individual brings a unique set of skills, expertise and mindset. Therefore we are happy to invite anyone who recognizes themselves in the profile to apply, even if you do not meet all the requirements.

# To learn more about the vacancy and to apply

If you said yes to most of the previous statements, then *send the following three document before August 7th, 2026*, to Sofia Gil-Clavel ([B.S.GilClavel@vu.nl](mailto:B.S.GilClavel@vu.nl?subject=AI Dilemma vacancy)) with the subject "AI Dilemma vacancy":

* your CV.
* your working plan. You can check this example on how to write it: [https://societal-analytics.nl/services/internship-opportunities/](https://societal-analytics.nl/services/internship-opportunities/)

Interviews will take place online between August 10th and August 14th, 2026.


If you have any questions feel free to reach out to Dr. Sofia Gil-Clavel ([B.S.GilClavel@vu.nl](mailto:B.S.GilClavel@vu.nl?subject=AI Dilemma vacancy)) with the subject "AI Dilemma vacancy".



